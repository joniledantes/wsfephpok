...
class pdf_eratosthene extends ModelePDFCommandes
{
		...
		if ($conf->commande->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				if ($notetoshow) {
                    ...
                    //Inicio cambios Manuel
					//$pdf->SetFont('', '', $default_font_size - 2);
					//$pdf->SetFont('', '', $default_font_size - 1);
					//$pdf->writeHTMLCell(190, 3, $this->posxdesc - 1, $tab_top, dol_htmlentitiesbr($notetoshow), 0, 1);
					//$pdf->writeHTMLCell(110, 3, $this->posxdesc - 1, 260, dol_htmlentitiesbr($notetoshow), 0, 1);
					//Fin cambios Manuel
				...
				if ($notetoshow) {
				    $pdf->SetFont('', '', $default_font_size - 2);
				    $pdf->writeHTMLCell(110, 3, $this->posxdesc - 1, 261, dol_htmlentitiesbr($notetoshow), 0, 1);
				}
	 ...
	 *   @return	void
	 */
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '', $outputlangsbis = null)
	{
		global $conf;
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null, $titlekey = "PdfOrderTitle")
	{
		...
		*/
		...
		$title = "ORDEN DE PEDIDO N°" . $outputlangs->convToOutputCharset($object->ref);
		$pdf->SetFont($this->posfac['factu_nro']['font'],$this->posfac['factu_nro']['style'],$this->posfac['factu_nro']['size']-2);
		$pdf->SetXY($this->posfac['factu_nro']['x'],$this->posfac['factu_nro']['y']);
		$pdf->MultiCell($this->posfac['factu_nro']['w'],$this->posfac['factu_nro']['h'],$title,0,$this->posfac['factu_nro']['alig']);
    	...
    	*/
		...
		$dlp = dol_print_date($object->delivery_date, "daytext", false, $outputlangs, true);
		$pdf->SetFont($this->posfac['factu_cpago']['font'], $this->posfac['factu_cpago']['style'], $this->posfac['factu_cpago']['size']);
		$pdf->SetXY($this->posfac['factu_cpago']['x'], (int)$this->posfac['factu_cpago']['y'] + 5);
		...
		$lib_availability = str_replace('\n', "\n", $lib_availability);
		$pdf->SetFont($this->posfac['factu_cpago']['font'], $this->posfac['factu_cpago']['style'], $this->posfac['factu_cpago']['size']);
		$pdf->SetXY($this->posfac['factu_cpago']['x'], (int)$this->posfac['factu_cpago']['y'] + 9);
