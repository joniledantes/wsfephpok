...
//        $texte.= '<input type="hidden" name="maskconstboleta" value="FACTURE_MERCURE_MASK_BOLETA">';
...
//        $texte.= '<td align="right">'.$form->textwithpicto('<input type="text" class="flat" size="24" name="maskboleta" value="'.$conf->global->FACTURE_MERCURE_MASK_BOLETA.'">',$tooltip,1,1).'</td>';
//        $texte.= '</tr>';       
...
//        else if (is_object($facture) && $facture->type == 10) $mask=$conf->global->FACTURE_MERCURE_MASK_BOLETA;
// BOLETA JONI
      ...
      //WSFE  
//      	if ($facture->modelpdf == 'fe' && $facture->mode_reglement_code!="ECO" && $facture->mode_reglement_code!="PRO")   //agregar ExtraField en fatura llamado fe
...
//        if ($facture->mode_reglement_code=="ECO") $mask="PROF-".$conf->global->FACTURE_MERCURE_MASK_INVOICE;	
if ($facture->mode_reglement_code=="ECO") $mask="PROF-".explode('-',$conf->global->FACTURE_MERCURE_MASK_INVOICE)[2];
          
//            if ($facture->mode_reglement_code=="ECO" || $facture->mode_reglement_code=="PRO") $mask="PROF-".$conf->global->FACTURE_MERCURE_MASK_INVOICE;	
//            if ($facture->mode_reglement_code=="PRO") $mask="PROF-".$conf->global->FACTURE_MERCURE_MASK_INVOICE;	
