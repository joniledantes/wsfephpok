		...
		*/
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
               ...
               //Cargo Plantilla
				$pagecount = $pdf->setSourceFile(DOL_DOCUMENT_ROOT.'/wsfephp/plantillas/factura.pdf');
				  ...
				  // Se agregaron estas lineas por que el la funcion pdf_getlinetotalwithtax y pdf_getlineupwithtax mo chequea signo
                   ...
                   ///-------------------
					...
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
					{
						...
						$pdf->MultiCell($this->posfac['item_ivarate']['w'], $this->posfac['item_ivarate']['h'], $vat_rateaimprimir, 0, $this->posfac['item_ivarate']['alig']);
						...
						// Fin Manuel
						$pdf->MultiCell($this->posfac['item_iva']['w'], $this->posfac['item_iva']['h'], $tvataimprimir, 0, $this->posfac['item_iva']['alig']);
					...
					if ($this->wsfe->cbttipo == '1' or $object->thirdparty->typent_code =='A') {
						...
						$pdf->MultiCell($this->posfac['item_prec']['w'], $this->posfac['item_prec']['h'], '$' . number_format(($object->lines[$i]->total_ttc/$object->lines[$i]->qty)/((100-$object->lines[$i]->remise_percent)/100), 2, ',', '.'), 0, $this->posfac['item_prec']['alig'], 0);
					}
					...
					$pdf->MultiCell($this->posfac['item_imp']['w'], $this->posfac['item_imp']['h'], '$' . $total_excl_tax_aimprimir, 0, $this->posfac['item_imp']['alig'], 0);
...
*/
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if ($this->wsfe->cbttipo =='1' or $object->thirdparty->typent_code=='A') {
		  ...
		  $pdf->MultiCell($this->posfac['tot_col1']['w'],$this->posfac['tot_col1']['h'], $outputlangs->transnoentities("TotalHT"), 0, $this->posfac['tot_col1']['alig'], 1);
			//$pdf->MultiCell($largcol2, $tab2_hl, price($sign * ($object->total_ht + (!empty($object->remise) ? $object->remise : 0)), 0, $outputlangs), 0, 'R', 1);
		  $pdf->SetXY($col2x, $tab2_top + 0);
		  $pdf->MultiCell($this->posfac['tot_col2']['w'],$this->posfac['tot_col2']['h'], '$' . price($sign * ($object->total_ht + (!empty($object->remise) ? $object->remise : 0)), 0, $outputlangs), 0, $this->posfac['tot_col2']['alig'], 1);
    	}
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			else
			{
				...
				$pdf->MultiCell($this->posfac['tot_col1']['w'], $tab2_hl, $outputlangs->transnoentities("TotalTTC"), $useborder, 'L', 1);
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				//$pdf->MultiCell($largcol2, $tab2_hl, price($sign * $object->total_ttc, 0, $outputlangs), $useborder, 'R', 1);
				$pdf->MultiCell($this->posfac['tot_col2']['w'],$tab2_hl, '$' . price($sign * $object->total_ttc, 0, $outputlangs), $useborder, 'R', 1);
			}
		}
		...
		*/
		...
		$nexY += 6; 
		$pdf->Image($qrpath, 15, 255, 0, 30);	// width=0 (auto)
		...
		$pdf->Multicell(250,3,$estado,0,'L');

		/*
		$pdf->SetXY(50, $posy+7.5);
		$pdf->SetFont('helvetica','I', 7);
		$pdf->Multicell(350,3,'La Administración Federal no se responsabiliza por los datos ingresados en el detalle de la operación','','L');
		*/
		
		$pdf->SetXY(50,$posy+8);
		$pdf->SetFont('helvetica','I', 8);
		$pdf->Multicell(150,3,'La Administración Federal no se responsabiliza por los','','L');

		$pdf->SetXY(50,$posy+11);
		$pdf->SetFont('helvetica','N', 8);
		$pdf->Multicell(150,3,'datos ingresados en el detalle de la operación','','L');


		$pdf->SetXY(50,$posy+15.5);
		$pdf->SetFont('helvetica','N', 9);
		$pdf->Multicell(350,3,$mensajecae,'','L');
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs,$intCopias=1)
	{
		...
		*/
		...
		/*if ($conf->global->WSFE_FAC_PDF_PRINT_LABEL)*/ $label= 'Condición: ';
