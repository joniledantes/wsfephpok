...
//MANU
if(isset($object->pos_facture) && !empty($object->pos_facture)) {
    $sql = "SELECT pf.fk_cash";
    $sql.= " FROM ".MAIN_DB_PREFIX."pos_facture as pf";
    $sql.= " WHERE pf.fk_facture = ". $object->ref;
    $result = $db->query($sql);
    $result = $result->fetch_array();
    $ptovtaCash = intval($result[0]);
    $varGlobalPosSource = "TAKEPOS_PTOVTA" . $ptovtaCash;
    if(isset($conf->global->$varGlobalPosSource) && !empty($conf->global->$varGlobalPosSource)) {
        $ptovta = $conf->global->$varGlobalPosSource; // Pto de venta segun constante asociada a Terminal Pos
    } else {
