   ...
   if ($conf->global->MAIN_INFO_SOCIETE_FORME_JURIDIQUE !=2301){  //Monotributista
    ...
    if ($object->thirdparty->typent_code == "A") {
		...
		if ($object->type == 0){
			...
			$tipo_cbte = 2;		
//
        }elseif ($object->type == 10) { //proforma A
            $typeent="PRO-";
            $tipo_cbte = 0; 
//            
		}
		...
		if ($object->type == 0){
			...
			$tipo_cbte = 7;		
//
        }elseif ($object->type == 10) { //nota de debito B
            $typeent="PRO-";
            $tipo_cbte = 0; 
//				
		}	 
...
if($object->fk_facture_source > 0) {
    $regfe['CbtesAsoc'] = [
        'CbteAsoc' => [
            ...
            'Cuit' => $nro_doc,
            //'CbteFch' => "20211227"
            'CbteFch' => $fechaCbteAsoc //intval(date('Ymd', strtotime($fechaAsoc)))
        ]
    ];
}
/*
    $regfe['CbteAsoc'] = array(
        'Tipo' => 1,
        'PtoVta' => 6,
        'Nro' => 3,
        'Cuit' => $nro_doc,
    ...
    */
    //$regfe['cbtasocs'] = ["cuit"=>$nro_doc, "fecha"=>"20211227", "nro"=>3, "pto_vta"=>6,"tipo"=>1];
    /*
    $regfe['CbtesAsoc'] = array("cuit"=>$nro_doc, "fecha"=>"20211227", "nro"=>3, "pto_vta"=>6,"tipo"=>1);
    //$regfeasoc = array("cuit"=>$nro_doc, "fecha"=>"20211227", "nro"=>3, "pto_vta"=>6,"tipo"=>1);
    //FIN TODO MANUEL
